...
IMAP_PASSWORD = os.getenv('IMAP_PASSWORDT')
IMAP_FOLDER =  'INBOX'
...
def list_unread_emails():
    """Rerutn a bullet list of every UNREAD message' s UID, subject, date and sender"""
    ...
    response = json.dumps([
        {
            'uid': mail.uid,
            'date': mail.date.astimatezone().strftime('%Y-%m-%d %H:%M'),
...
def summarize_email(uid):
    """Summarize an e-mail given it's IMAP UID. Return a short summary of the e-mails content / body in plain text."""
